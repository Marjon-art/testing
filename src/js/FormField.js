...
import defaultPaySystem from "./defaultPaySystem";
...
export default class FormField {
  ...
  onChangeInput() {
    ...
    if (value.length > 0) {
      const arrIndex = defaultPaySystem(value);
      if (typeof arrIndex[0] === "string") {
        this.setMessage(arrIndex);
        this.cards.inactiveAll();
      } else {
        this.cards.activeSome(arrIndex);
      }
    }
  }
  ...
  onSubmit(e) {
    e.preventDefault();
    this.deleteMessage();
    const isValid = validateCardNumber(this.inputEl.value.trim());
    isValid
